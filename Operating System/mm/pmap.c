...
struct Page *pages; // the first page of the whole memory page linked list
...
void mips_detect_memory()
{
    ...
    maxpa = (u_long)0x4000000;   // 64MB = 1<<26
    basemem = (u_long)0x4000000; // 64MB
    // Step 2: Calculate corresponding npage value.
    npage = (u_long)16384; // basemem / 4K
    extmem = (u_long)0;
...
static void *alloc(u_int n, u_int align, int clear)
{
    ...
    freemem = freemem + n; // alloc mem upward
...
/*Overview:
    ...
    Use permission bits `perm` for the pgdir entries.
	Use permission bits `perm|PTE_V` for the pgtable entries.

  Pre-Condition:
	Size is a multiple of BY2PG.*/
void boot_map_segment(Pde *pgdir, u_long va, u_long size, u_long pa, int perm)
{
    u_long i, va_temp, pa_temp;
    Pte *pgtable_entry;
    ...
    *(pgdir + PDX(va)) |= perm; // all va_temp is in A page table
}
...
/*Overview:
 	Initialize memory free page list.
...
void page_init(void)
{
    /* Step 1: use LIST_INIT to initialize page_free_list. */
    LIST_INIT(&page_free_list); // page_free_list is a structure instance rather than a pointer
    /* Step 2: Align `freemem` up to multiple of BY2PG. */
    freemem = ROUND(freemem, BY2PG); // freemem is alloced upward so it should be rounded up
    /* Step 3: Mark all memory below `freemem` as used(set `pp_ref`filed to 1) */
    u_long i;
    u_long used_page_cnt = PADDR(freemem) / BY2PG;
    for (i = 0; i < used_page_cnt; i++)
    {
        (pages + i)->pp_ref = 1;
    }
    /* Step 4: Mark the other memory as free. */
    for (i = used_page_cnt; i < npage; i++)
    {
        struct Page *tmp_page = (pages + i);
        tmp_page->pp_ref = 0;
        LIST_INSERT_HEAD(&page_free_list, tmp_page, pp_link); // insert head is faster than tail
    }
}
...
{
    struct Page *ppage_temp; // the PHYSICAL page structure instance for pp to points at
    /* Step 1: Get a page from free memory. If fails, return the error code.*/
    if ((ppage_temp = LIST_FIRST(&page_free_list)) == NULL)
    {
        return (int)-E_NO_MEM;
    }
    /* Step 2: use `bzero` to initialize this page.*/
    LIST_REMOVE(ppage_temp, pp_link);
    bzero((void *)page2kva(ppage_temp), BY2PG);
    *pp = ppage_temp; // pp hasn't been referred yet, so the ref stays zero.
    return 0;
}
...
/*Overview:
    ...
    Caution: ONLY walk to pgt_entryp, so no need to fill pgt_entryp.
...
{
    Pde *pgdir_entryp;   // pointer of entry in pgdir for va
    Pte *pgtable;        // pointer of start of pgtable
    Pte *pgtable_entryp; // pointer of entry in pgtable
    struct Page *ppage;  // pointer of a physical page
    ...
    *ppte = NULL;
     ...
     * is set, create one. And set the correct permission bits for this new page
     * table.
     * Caution: When creating new page table, maybe out of memory. */
    if ((*pgdir_entryp & PTE_V) == 0)
    {
        if (create)
        {
...
/*Overview:
    insert physical page 'pp' in pgtable for virtual address 'va'.
...
int page_insert(Pde *pgdir, struct Page *pp, u_long va, u_int perm)
{
    ...
    pgdir_walk(pgdir, va, 0, &pgtable_entryp);

    if (pgtable_entryp != NULL && (*pgtable_entryp & PTE_V) != 0)
    {
        if (pa2page(*pgtable_entryp) != pp)
        {
        ...
        else
        {
            ...
            *pgtable_entryp = (page2pa(pp) | PERM);
            return 0;
        }
    }
    ...
    if (pgdir_walk(pgdir, va, 1, &pgtable_entryp))
    {
        return -E_NO_MEM;
    }
