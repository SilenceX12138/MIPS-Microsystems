...
void init_disk()
{
    ...
    nbitblock = (NBLOCK + BIT2BLK - 1) / BIT2BLK; // nbitblock : how many blocks are used to save bitmap
    nextbno = 2 + nbitblock;

    // Step 2: Initialize bitmap blocks.
    for (i = 0; i < nbitblock; ++i)
    {
        disk[2 + i].type = BLOCK_BMAP; // BLOCK2 (even 3,4,...) is used for bitmap
    }
    for (i = 0; i < nbitblock; ++i)
    {
        memset(disk[2 + i].data, 0xff, NBLOCK / 8); // NBLOCK / 8 is the byte count of valid blocks
    }
...
struct File *create_file(struct File *dirf)
{
    struct File *dirblk;
    int i, bno, found;
    int nblk = dirf->f_size / BY2BLK; // number of blocks that are linked under dirf now
    ...
    else
    {
        ...
        {
            bno = *(disk[dirf->f_indirect].data + nblk - 10);
        }
        dirblk = (struct File *)(disk[bno].data);
        for (i = 0; i < 16; i++)
        {
            struct File *tmpf = dirblk + i;
            if (strlen(tmpf->f_name) == 0)
            {
                return tmpf;
            }
        }
