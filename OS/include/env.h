...
struct Env
{
	struct Trapframe env_tf; // registers saving context when shifting processes
	LIST_ENTRY(Env)
	env_link;			 // Free PCB list
	u_int env_id;		 // Unique environment identifier
	u_int env_parent_id; // env_id of this env's parent
	u_int env_status;	 // Status of the environment : FREE/BLOCKED/RUNNABLE
	Pde *env_pgdir;		 // Kernel virtual address of page dir
	u_int env_cr3;		 // physical address of page dir
	LIST_ENTRY(Env)
	env_sched_link; // RUNNABLE PCB list
	u_int env_pri;	// priority of the process
	// Lab 4 IPC
