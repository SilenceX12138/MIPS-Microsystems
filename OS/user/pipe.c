...
piperead(struct Fd *fd, void *vbuf, u_int n, u_int offset)
{
	...
	// use fd to get pipe
	p = (struct Pipe *)fd2data(fd);
	// if pipe is empty and pipe is closed, return 0.
	...
	// read data byte by byte, and if empty pipe occured, return now pos.
	rbuf = (char *)vbuf;
	for (i = 0; i < n && (p->p_rpos < p->p_wpos); i++, p->p_rpos++)
	{
		rbuf[i] = p->p_buf[p->p_rpos % BY2PIPE]; // rbuf doesn't need to mod
	}
...
pipewrite(struct Fd *fd, const void *vbuf, u_int n, u_int offset)
{
	...
	// write data byte by byte unless it is full or all data is used
	wbuf = (char *)vbuf;
	for (i = 0; i < n; i++, p->p_wpos++)
	{
		...
		p->p_buf[p->p_wpos % BY2PIPE] = wbuf[i]; // wbuf doesn't need to mod
	}
