...
u_int diskaddr(u_int blockno)
{
	...
	{
		user_panic("diskaddr has problem 1\n");
	}
...
int map_block(u_int blockno)
{
	...
	int r = syscall_mem_alloc(0, diskaddr(blockno), PTE_V | PTE_R);
	if (r)
	{
		return r;
	}
...
void unmap_block(u_int blockno)
{
	...
	if (r)
	{
		user_panic("unmap_block has problem 2\n");
	}
...
int block_is_free(u_int blockno)
{
	...
	// blockno / 32 is the sub-bitmap location in bitmap array
	// blockno % 32 is the bit location in a word
...
void free_block(u_int blockno)
{
	...
	if ((super == 0) || (blockno >= super->s_nblocks))
	{
		user_panic("free_block has problem 1\n");
	}
...
int dir_lookup(struct File *dir, char *name, struct File **file)
{
	...
	nblock = ROUND(dir->f_size, BY2BLK) / BY2BLK; // nblock needs ROUND UP
